...
import pisi.unitedmeows.yystal.networking.simple.client.extension.impl.CTcpFixedSize;
...
public class YTcpClient implements IDisposable {
    ...
    public void receiveBuffered(BiConsumer<byte[], Boolean> bufferReceive) {
        Async.async(() -> {
            System.out.println("started receiving");
            byte[] sizeRaw = readNext();
            final int totalSize;
            int size1;
            try {
                size1 = new MemoryReader(sizeRaw).readInt();
            } catch (IOException e) {
                e.printStackTrace();
                size1 = 0;
            }
            ...
            totalSize = size1;
            ref<Integer> received = YYStal.reference(0);
            System.out.println("test");

            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    boolean finished =  received.get() >= totalSize;
                    while (!finished) {
                        byte[] data = readNext();
                        received.set(received.get() + data.length);
                        finished = received.get() >= totalSize;
                        bufferReceive.accept(data, finished);
                    }
                }
            });
            thread.start();
