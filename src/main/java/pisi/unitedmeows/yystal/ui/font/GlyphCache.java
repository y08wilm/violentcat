...
public class GlyphCache {
	/**
	 * The width in pixels of every texture used for caching pre-rendered glyph images. Used by
	 * GlyphCache when calculating floating point 0.0-1.0 texture coordinates. Must be a power of two
	 * for mip-mapping to work.
	 */
	private static final int TEXTURE_WIDTH = 256;
	/**
	 * The height in pixels of every texture used for caching pre-rendered glyph images. Used by
	 * GlyphCache when calculating floating point 0.0-1.0 texture coordinates. Must be a power of two
	 * for mip-mapping to work.
	 */
	...
	/**
	 * The width in pixels of a transparent border between individual glyphs in the cache texture. This
	 * border keeps neighboring glyphs from "bleeding through" when the scaled GUI resolution is not
	 * pixel aligned and sometimes results in off-by-one sampling of the glyph cache textures.
	 */
	...
	/**
	 * The Graphics2D associated with stringImage and used for string drawing to extract the individual
	 * glyph shapes.
	 */
	private Graphics2D stringGraphics;
	/**
	 * All font glyphs are packed inside this image and are then loaded from here into an OpenGL
	 * texture.
	 */
	...
	/**
	 * A big-endian direct int buffer used with glTexSubImage2D() and glTexImage2D(). Used for loading
	 * the pre-rendered glyph images from the glyphCacheImage BufferedImage into OpenGL textures. This
	 * buffer uses big-endian byte ordering to ensure that the integers holding packed RGBA colors are
	 * stored into memory in a predictable order.
	 */
	...
	/**
	 * A single integer direct buffer with native byte ordering used for returning values from
	 * glGenTextures().
	 */
	...
	/**
	 * A list of all fonts that have been returned so far by lookupFont(), and that will always be
	 * searched first for a usable font before searching through allFonts[]. This list will only have
	 * plain variation of a font at a dummy point size, unlike fontCache which could have multiple
	 * entries for the various styles (i.e. bold, italic, etc.) of a font. This list starts with Java's
	 * "SansSerif" logical font.
	 */
	private final List<Font> usedFonts = new ArrayList<>();
	/**
	 * ID of current OpenGL cache texture being used by cacheGlyphs() to store pre-rendered glyph
	 * images.
	 */
	private int textureName;
	/**
	 * A cache of all fonts that have at least one glyph pre-rendered in a texture. Each font maps to an
	 * integer (monotonically increasing) which forms the upper 32 bits of the key into the glyphCache
	 * map. This font cache can include different styles of the same font family like bold or italic.
	 */
	private final LinkedHashMap<Font, Integer> fontCache = new LinkedHashMap<>();
	/**
	 * A cache of pre-rendered glyphs mapping each glyph by its glyphcode to the position of its
	 * pre-rendered image within the cache texture. The key is a 64 bit number such that the lower 32
	 * bits are the glyphcode and the upper 32 are the index of the font in the fontCache. This makes
	...
	/**
	 * The X coordinate of the upper=left corner in glyphCacheImage where the next glyph image should be
	 * stored. Glyphs are always added left-to-right on the curren tline until it fills up, at which
	 * point they continue filling the texture on the next line.
	 */
	private int cachePosX = GLYPH_BORDER;
	/**
	 * The Y coordinate of the upper-left corner in glyphCacheImage where the next glyph image should be
	 * stored. Glyphs are stored left-to-right in horizontal lines, and top-to-bottom until the entire
	 * texture fills up. At that point, a new texture is allocated to keep storing additional glyphs,
	 * and the original texture remains allocated for the lifetime of the application.
	 */
	private int cachePosY = GLYPH_BORDER;
	/**
	 * The height in pixels of the current line of glyphs getting written into the texture. This value
	 * determines by how much cachePosY will get incremented when the current horizontal line in the
	 * texture fills up.
	 */
	...
	GlyphCache() {
		...
		/* The drawImage() to this buffer will copy all source pixels instead of alpha blending them into
		 * the current image */
		...
		/* Use Java's logical font as the default initial font if user does not override it in some
		 * configuration file */
	 ...
	 * to StringCache. This method is normally called by StringCache.setDefaultFont() since the
	 * StringCache must also invalidate itself so it can re-layout and re-cache the glyphs for all
	 * strings using the new font.
	 *
	 * @param size the new point size
	 */
	Font setDefaultFont(final String name, final float size, final boolean antiAlias) {
		return setDefaultFont(getPathFromAssets(name + ".ttf"), size, antiAlias);
	}

    Font setDefaultFont(final InputStream stream, final float size, final boolean antiAlias) {
        usedFonts.clear();
        Font f = null;
        try {
	...
	/**
	 * Find the first font in the system able to render at least one character from a given string. The
	 * function always tries searching first in the fontCache (based on the request style). Failing
	 * that, it searches the usedFonts list followed by the allFonts[] array.
	 *
	 * @param text  the string to check against the font
	 * @param start the offset into text at which to start checking characters for being supported by a
	 *              font
	 * @param limit the (offset + length) at which to stop checking characters
	 * @param style combination of the Font.PLAIN, Font.BOLD, and Font.ITALIC to request a particular
	 *              font style
	 *
	 * @return an OpenType font capable of displaying at least the first character at the start position
	 *         in text
	 */
	Font lookupFont(final char[] text, final int start, final int limit, final int style) {
		/* Try using an already known base font; the first font in usedFonts list is the one set with
		 * setDefaultFont() */
		...
		while (iterator.hasNext()) {
			...
			if (font.canDisplayUpTo(text, start, limit) != start) /* Return a font instance of the proper point size and style; usedFonts has only 1pt sized plain
			 * style fonts */
				return font.deriveFont(style, fontSize);
		}
		/* If still not found, try searching through all fonts installed on the system for the first that
		 * can layout this string */
		...
		while (iterator.hasNext()) {
			...
			if (font.canDisplayUpTo(text, start, limit) != start) {
				...
				usedFonts.add(font);
				/* Return a font instance of the proper point size and style; allFonts has only 1pt sized plain
				 * style fonts */
				return font.deriveFont(style, fontSize);
			}
		}
		/* If no supported fonts found, use the default one (first in usedFonts) so it can draw its unknown
		 * character glyphs */
		final Font font = usedFonts.get(0);
		/* Return a font instance of the proper point size and style; usedFonts only 1pt sized plain style
		 * fonts */
		return font.deriveFont(style, fontSize);
	}

	/**
	 * Given an OpenType font and a glyph code within that font, locate the glyph's pre-rendered image
	 * in the glyph cache and return its cache entry,. The entry stores the texture ID with the
	 * pre-rendered glyph image, as well as the position and size of that image within the texture. This
	 * function assumes that any glyph lookup requests passed to it have been already cached by an
	 * earlier call to cacheGlyphs().
	 *
	 * @param font      the font to which this glyphCode belongs and which was used to pre-render the
	 *                  glyph image in cacheGlyphs()
	...
	/**
	 * Given an OpenType font and a string, make sure that every glyph used by that string is
	 * pre-rendered into an OpenGL texture and cached in the glyphCache map for later retrieval by
	 * lookupGlyph()
	 *
	 * @param font        the font used to create a GlyphVector for the string and to actually draw the
	 *                    individual glyphs
	 ...
	 * @param layoutFlags either Font.LAYOUT_RIGHT_TO_LEFT or Font.LAYOUT_LEFT_TO_RIGHT; needed for weak
	 *                    bidirectional characters like parenthesis which are mapped to two different
	 *                    glyph codes depending on the surrounding text direction
	 */
	// todo May need a blank border of pixels around everything for mip-map/tri-linear filtering with
	// Optifine
	void cacheGlyphs(final Font font, final char[] text, final int start, final int limit, final int layoutFlags) {
		/* Create new GlyphVector so glyphs can be moved around (kerning workaround; see below) without
		 * affecting caller */
		final GlyphVector vector = layoutGlyphVector(font, text, start, limit, layoutFlags);
		/* Pixel aligned bounding box for the entire vector; only set if the vector has to be drawn to cache
		 * a glyph image */
		Rectangle vectorBounds = null;
		/* This forms the upper 32 bits of the fontCache key to make every font/glyph code point unique */
		final long fontKey = (long) fontCache.get(font) << 32;
		final int numGlyphs = vector.getNumGlyphs(); /* Length of the GlyphVector */
		...
		for (int index = 0; index < numGlyphs; index++) {
			 ...
			 * Davengari glyphs under Windows 7. The vectorRendered flag will draw the string at most one
			 * time. */
			if (!vectorRendered) {
				vectorRendered = true;
				/* Kerning can make it impossible to cleanly separate adjacent glyphs. To work around this, each
				 * glyph is manually advanced by 2 pixels to the right of its neighbor before rendering the entire
				 * string. The getGlyphPixelBounds() later on will return the new adjusted bounds for the glyph. */
				for (int i = 0; i < numGlyphs; i++) {
					final Point2D pos = vector.getGlyphPosition(i);
					pos.setLocation(pos.getX() + 2 * i, pos.getY());
					vector.setGlyphPosition(i, pos);
				}
				/* Compute the exact area that the rendered string will take up in the image buffer. Note that the
				 * string will actually be drawn at a positive (x,y) offset from (0,0) to leave enough room for the
				 * ascent above the baseline and to correct for a few glyphs that appear to have negative horizontal
				 * bearing (e.g. U+0423 Cyrillic uppercase letter U on Windows 7). */
			...
			/* Get the glyph's pixel-aligned bounding box. The JavaDoc claims that the "The outline returned by
			 * this method is positioned around the origin of each individual glyph." However, the actual bounds
			 * are all relative to the start of the entire GlyphVector, which is actually more useful for
			 * extracting the glyph's image from the rendered string. */
			...
			/* If the entire image is full, update the current OpenGL texture with everything changed so far in
			 * the image (i.e. the dirty rectangle), allocate a new cache texture, and then continue storing
			 * glyph images to the upper-left corner of the new texture. */
			...
			glyphCacheGraphics.drawImage(stringImage, cachePosX, cachePosY, cachePosX + rect.width, cachePosY + rect.height, rect.x, rect.y, rect.x + rect.width, rect.y + rect.height, null);
			/* Store this glyph's position in texture and its origin offset. Note that "rect" will not be
			 * modified after this point, and getGlyphPixelBounds() always returns a new Rectangle. */
			rect.setLocation(cachePosX, cachePosY);
			/* Create new cache entry to record both the texture used by the glyph and its position within that
			 * texture. Texture coordinates are normalized to 0.0-1.0 by dividing with TEXTURE_WIDTH and
			 * TEXTURE_HEIGHT. */
			...
			/* The lower 32 bits of the glyphCache key are the glyph codepoint. The upper 64 bits are the font
			 * number stored in the fontCache. This creates a unique numerical id for every font/glyph
			 * combination. */
			glyphCache.put(fontKey | glyphCode, entry);
			/* Track the overall modified region in the texture by performing a union of this glyph's texture
			 * position with the update region created so far. Reusing "rect" here makes it easier to extend the
			 * dirty rectangle region than using the add(x, y) method to extend by a single point. Also note
			 * that creating the first dirty rectangle here avoids having to deal with the special rules for
	 ...
	 *
	 * @param dirty The rectangular region in glyphCacheImage that has changed and needs to be copied
	 *              into the texture bleedover when interpolation is active or add a small "fudge
	 *              factor" to the UV coordinates like already n FontRenderer
	 */
	// todo Add mip-mapping support here
	// todo Test with bilinear texture interpolation and possibly add a 1 pixel transparent border
	// around each glyph to avoid
	...
	/**
	 * Allocte and initialize a new BufferedImage and Graphics2D context for rendering strings into. May
	 * need to be called at runtime to re-allocate a bigger BufferedImage if cacheGlyphs() is called
	...
	private void allocateStringImage(final int width, final int height) {
		...
		/* Full white (1.0, 1.0, 1.0, 1.0) can be modulated by vertex color to produce a full gamut of text
		 * colors, although with a GL_ALPHA8 texture, only the alpha component of the color will actually
		 * get loaded into the texture. */
		stringGraphics.setPaint(Color.WHITE);
	}

	/**
	 * Set rendering hints on stringGraphics object. Uses current antiAliasEnabled settings and is
	 * therefore called both from allocateStringImage() when expanding the size of the BufferedImage and
	...
	private void setRenderingHints() {
		...
		stringGraphics.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_NORMALIZE);
		stringGraphics.setRenderingHint(RenderingHints.KEY_DITHERING, RenderingHints.VALUE_DITHER_DISABLE);
		stringGraphics.setRenderingHint(RenderingHints.KEY_COLOR_RENDERING, RenderingHints.VALUE_COLOR_RENDER_QUALITY);
		stringGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_LCD_HRGB);
		stringGraphics.setRenderingHint(RenderingHints.KEY_TEXT_LCD_CONTRAST, 120);
		//		stringGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, antiAliasEnabled ? RenderingHints.VALUE_ANTIALIAS_ON : RenderingHints.VALUE_ANTIALIAS_OFF);
		//		stringGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, antiAliasEnabled ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON : RenderingHints.VALUE_TEXT_ANTIALIAS_OFF);
		//		stringGraphics.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_OFF);
	}

	/**
	 * Allocate a new OpenGL texture for caching pre-rendered glyph images. The new texture is
	 * initialized to fully transparent white so the individual glyphs images within can have a
	 * transparent border between them. The new texture remains bound after returning from the function.
	 */
	...
	private void allocateGlyphCacheTexture() {
		...
		/* Initialize texture with the now cleared BufferedImage. Using a texture with GL_ALPHA8 internal
		 * format may result in faster rendering since the GPU has to only fetch 1 byte per texel instead of
		 * 4 with a regular RGBA texture. */
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, textureName);
	 ...
	 * glText(Sub)Image2D(). This function takes care of converting the ARGB format used with
	 * BufferedImage into the RGBA format used by OpenGL.
	 *
	...
	private void updateImageBuffer(final int x, final int y, final int width, final int height) {
		/* Copy raw pixel data from BufferedImage to imageData array with one integer per pixel in
		 * 0xAARRGGBB form */
	...
	public static class Glyph implements Comparable<Glyph> {
		/**
		 * The index into the original string (i.e. with color codes) for the character that generated this
		 * glyph.
		 */
		public int stringIndex;
		/** Texture ID and position/size of the glyph's pre-rendered image within the cache texture. */
		public Entry texture;
