...
public class YWindow extends YContainer {
	...
	private void _open() {
            ...
            if (action == GLFW_PRESS) {
                ...
                for (YElement element : elements()) {
                    if (element.isMouseOver(mouseX(), mouseY())) {
                        element.focus();
                        System.out.println("Focusing " + element.getClass().getSimpleName());
                        clickedElement = element;
                        focused.set(element);
                        break;
                    }
                }
                clickedElement._mouseClick(button);
            } else if (action == GLFW_RELEASE) {
                if (clickedElement != null) {
                    clickedElement._mouseRelease(button);
                }
            }
		...
		// center the window
		// add window start options like (CENTER, RANDOM, LEFT, RIGHT)
	...
	private void loop() {
        ...
        YShader shader = YUI.loadShader("basic");
    ...
    private boolean shouldWindowClose() {
        if (glfwWindowShouldClose(window)) {
            if (exitEvent != null) {
                ref<Boolean> result = YYStal.reference(false);
                exitEvent.fire(result);
                glfwSetWindowShouldClose(window, !result.get());
                return !result.get();
            }
            return true;
        }
